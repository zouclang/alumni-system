...
export default function CityPicker({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  ...
  useEffect(() => {
    if (value) {
      if (value.includes('-')) {
        ...
        // Try to guess from map
        let foundProv = '';
        for (const p in PROVINCE_CITY_MAP) {
          if (value.startsWith(p) || PROVINCE_CITY_MAP[p].some(c => value.includes(c) || c.includes(value))) {
            foundProv = p;
            break;
          }
        }
        if (foundProv) {
          setProv(foundProv);
          // If the whole value is just province name, city can be empty or the first city
          const matchedCity = PROVINCE_CITY_MAP[foundProv].find(c => value.includes(c) || c.includes(value));
          if (matchedCity) setCity(matchedCity);
          else setCity(value.replace(foundProv, ''));
        }
      }
    }
  }, [value]);
  ...
  return (
    <div className="form-group">
      <label className="form-label">家乡 (省/市)</label>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
        <select className="form-select" value={prov} onChange={handleProvChange}>
          <option value="">省份/直辖市</option>
          {Object.keys(PROVINCE_CITY_MAP).map(p => <option key={p} value={p}>{p}</option>)}
        </select>
        <select className="form-select" value={city} onChange={handleCityChange} disabled={!prov}>
          <option value="">城市/区</option>
          {prov && PROVINCE_CITY_MAP[prov]?.map(c => <option key={c} value={c}>{c}</option>)}
        </select>
      </div>
    </div>
  );
