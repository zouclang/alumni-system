...
> 项目使用 Next.js `standalone` 输出模式，请严格按照以下步骤操作。
...
```bash
pm2 status                    # 查看运行状态
pm2 logs alumni-system        # 查看实时日志
pm2 restart alumni-system     # 重启应用
pm2 stop alumni-system        # 停止应用
```

> 默认监听端口为 **8085**，请确保服务器安全组已放行该端口。
